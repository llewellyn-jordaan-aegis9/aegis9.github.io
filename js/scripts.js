...
$(document).ready(function () {
    ...
    $("a").on('click', function () {
        var trgt = $(this).attr('href').toString();

        if (trgt.indexOf('#') > -1 && trgt.indexOf('-container') > -1) {
            $('.page-content').fadeOut(10, function () {
                setTimeout(function () {
                    $(trgt).removeClass('hidden').fadeIn(1000, function () {
                        $(window).scrollTop(-50);
                    });
...
function updateNav() {
    ...
    mr_navOuterHeight = 55;

    let scroll_pos = $(document).scrollTop();

    if (scroll_pos > mr_navOuterHeight + mr_fixedAt) {
        ...
        if (scroll_pos > mr_navOuterHeight) {
            if (!mr_navFixed) {
